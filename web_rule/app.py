...
# Secret key only needed because we now keep per-user scratch state in the
# session instead of a shared module-level dict (which broke under
# concurrent users). Override via env var in any real deployment.
app.secret_key = os.environ.get('FLASK_SECRET_KEY', os.urandom(24))

MAX_TABLES = 8  # guard against pathological exhaustive join-order search
...
def _count_base_tables(node, seen=None):
    """Rough guard so a pathological query can't blow up the exhaustive
    join-order/strategy search (3^(n-1) plans)."""
    if seen is None:
        seen = set()
...
def _parse_sql_to_json(sql_query: str):
    ...
    try:
        ...
        return None, {
            'success': False,
            'error': 'Parser output was not valid JSON. Check your SQL syntax.',
        }
    finally:
...
def health():
    parser_bin = _find_parser_binary()
    return jsonify({
        'status': 'ok' if parser_bin else 'degraded',
        'parser_binary_found': bool(parser_bin),
    })
...
def analyze_sql_pipeline():
    ...
    if len(_count_base_tables(parsed_plan)) > MAX_TABLES:
        return jsonify({
            'success': False,
            'error': f'Query references more than {MAX_TABLES} tables; '
                     'join-order search is exhaustive and would be too slow.',
        })
    ...
    # Stash only what later /optimize/* calls need, scoped to this user's
    # session rather than a shared global dict.
    session['pred_plan_json'] = pred_plan
...
def optimize_pred_push():
    ...
    session['pred_plan_json'] = result['optimized_plan_json']
    return jsonify({
        'success': True,
...
def optimize_join_cost():
    ...
    try:
        ...
        relational_algebra = session.get('pred_plan_json', relational_algebra)
...
if __name__ == '__main__':
    ...
    app.run(debug=debug_mode, port=int(os.environ.get('PORT', 8000)))
